<x-filament-panels::page @class([ 'fi-resource-edit-record-page' , 'fi-resource-view-record-page' , 'fi-resource-' .
    ...
    <x-filament-panels::form id="form" :wire:key="$this->getId() . '.forms.' . $this->getFormStatePath()"
        ...
        <x-filament-panels::form.actions :actions="$this->getCachedFormActions()" alignment="end"
    ...
    <div class="architeca-project-detail">
        <div class="architeca-kpi-grid" aria-label="Project key performance indicators">
            ...
            $latestProgress = $latestTask ? ($record->tasks()->sum('percentage_weight') ?
            ($record->tasks()->where('is_completed', true)->sum('percentage_weight') /
            $record->tasks()->sum('percentage_weight')) * 100 : 0) : 0;
            ...
            <div class="architeca-kpi-card architeca-kpi-attachments">
                <div class="architeca-kpi-label">
                    <x-filament::icon icon="heroicon-o-paper-clip" class="h-5 w-5" /><span>Attachments</span>
                </div>
        ...
        <div class="architeca-project-tabs-shell">
            <x-filament-panels::resources.relation-managers :active-locale="isset($activeLocale) ? $activeLocale : null"
                ...
                :content-tab-label="$this->getContentTabLabel()" :content-tab-icon="$this->getContentTabIcon()"
                :content-tab-position="$this->getContentTabPosition()" :managers="$relationManagers"
