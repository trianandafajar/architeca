    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div class="overflow-hidden rounded-xl border border-border bg-card shadow-sm xl:col-span-2">
            <div class="flex items-center justify-between border-b border-border px-6 py-4">
                ...
                <a href="{{ route('filament.contractor.resources.projects.index') }}"
                ...
                <table class="w-full min-w-[680px]">
                    ...
                    <tbody class="divide-y divide-border">
                        ...
                        <tr class="transition-colors hover:bg-muted/30">
                            <td class="px-6 py-4">
                                <a href="{{ route('filament.contractor.resources.projects.view', $project) }}"
                                    class="text-sm font-medium text-foreground hover:text-primary">{{ $project->name
                                    }}</a>
                                <p class="mt-1 text-xs text-muted-foreground">{{ $project->client_name ?: 'No client
                                    specified' }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="inline-flex rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset {{ $statusClasses[$project->status] ?? 'bg-gray-50 text-gray-700 ring-gray-600/20' }}">{{
            ...
            <div class="divide-y divide-border">
                ...
                <div class="flex items-start gap-3 px-6 py-4">
                    ...
                    <div class="min-w-0 flex-1">
                        <p class="truncate text-sm font-medium text-foreground">{{ $activity->project?->name ??
                            'Project' }}</p>
                        <p class="mt-1 line-clamp-2 text-xs text-muted-foreground">{{ $activity->work_description ?: 'No
                            description' }}</p>
                        <p class="mt-1 text-xs text-muted-foreground">{{ $activity->user?->name ?? 'Team member' }} · {{
                            $activity->report_date?->diffForHumans() }}</p>
                    </div>
                </div>
                @empty
        ...
        <div class="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="rounded-lg border border-border p-4">
                <div class="flex items-center justify-between gap-3">
                    <p class="truncate text-sm font-medium text-foreground">{{ $item['project']->name }}</p><span
                        class="text-xs font-semibold text-primary">{{ number_format($item['percentage'], 0) }}%</span>
                </div>
