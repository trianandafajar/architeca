<x-filament-panels::page>
    ...
    <div class="mb-6 flex flex-wrap gap-2 rounded-xl bg-white p-4 shadow-sm">
        ...
        <button wire:click="$set('tab', 'rab')" class="rounded-lg px-4 py-2 text-sm font-medium transition @if($tab === 'rab') bg-primary-500 text-white @else bg-gray-100 text-gray-700 hover:bg-gray-200 @endif">RAB / Budget</button>
    ...
    @if ($tab === 'overview')
        <div class="rounded-xl bg-white p-6 shadow-sm">
            <h2 class="mb-4 text-lg font-semibold text-gray-800">Overview Project</h2>
            <dl class="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div class="rounded-lg bg-gray-50 p-4">
                    <dt class="text-sm text-gray-500">Nama Project</dt>
                ...
                <div class="rounded-lg bg-gray-50 p-4">
                    ...
                    <dd class="mt-1 font-semibold text-gray-800">Rp {{ number_format($project->contract_value, 0, ',', '.') }}</dd>
                </div>
                <div class="rounded-lg bg-gray-50 p-4">
                ...
                <div class="rounded-lg bg-gray-50 p-4">
                    ...
                    <dd class="mt-1 font-semibold text-gray-800">{{ $project->start_date?->format('d M Y') }} — {{ $project->end_date?->format('d M Y') }}</dd>
                </div>
            </dl>
        </div>
    @endif
    ...
    @if ($tab === 'rab')
        <div class="rounded-xl bg-white p-6 shadow-sm">
            <h2 class="mb-4 text-lg font-semibold text-gray-800">RAB / Budget</h2>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-600">Harga Satuan</th>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @forelse ($project->budgetItems as $item)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-sm text-gray-800">Rp {{ number_format($item->unit_price, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-sm font-medium text-gray-800">Rp {{ number_format($item->total_price, 0, ',', '.') }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="px-4 py-4 text-center text-sm text-gray-500">Belum ada data RAB.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    @if ($tab === 'progress')
        <div class="rounded-xl bg-white p-6 shadow-sm">
            <h2 class="mb-4 text-lg font-semibold text-gray-800">History Progress</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @forelse ($project->progressUpdates as $pu)
                            <tr>
                                <td class="px-4 py-3 text-sm text-gray-800">{{ $pu->progress_date->format('d M Y') }}</td>
                        ...
                        @empty
                            <tr><td colspan="4" class="px-4 py-4 text-center text-sm text-gray-500">Belum ada data progress.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @forelse ($project->dailyReports as $dr)
                            <tr>
                                <td class="px-4 py-3 text-sm text-gray-800">{{ $dr->report_date->format('d M Y') }}</td>
                        ...
                        @empty
                            <tr><td colspan="4" class="px-4 py-4 text-center text-sm text-gray-500">Belum ada daily report.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @forelse ($project->expenses as $exp)
                            <tr>
                                <td class="px-4 py-3 text-sm text-gray-800">{{ $exp->expense_date->format('d M Y') }}</td>
                                <td class="px-4 py-3 text-sm">
                                ...
                                <td class="px-4 py-3 text-sm font-medium text-gray-800">Rp {{ number_format($exp->amount, 0, ',', '.') }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="4" class="px-4 py-4 text-center text-sm text-gray-500">Belum ada expenses.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        ...
                        @empty
                            <tr><td colspan="3" class="px-4 py-4 text-center text-sm text-gray-500">Belum ada anggota.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
